...
> **認証の仕組み**: claude.ai は登録時に Worker の `/oauth/token` エンドポイントへ
> `grant_type=client_credentials` でリクエストを送り、Bearer トークンを取得する。
> OAuth Secret が `MCP_AUTH_TOKEN` と一致する場合のみトークンが発行される。
